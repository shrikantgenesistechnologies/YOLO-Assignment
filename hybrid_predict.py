...
def postprocess(outputs, img_shape, preprocess_info, conf_threshold=0.15, iou_threshold=0.45):
    ...
    # Get max confidence and class id for each box
...
def hybrid_predict(pt_model_path, onnx_model_path, image_path, conf_threshold=0.15, iou_threshold=0.45, verbose=True):
    """
    Use both PyTorch and ONNX YOLO models for prediction, combining their results
    ...
    if not os.path.exists(pt_model_path):
        print(f"Error: PyTorch model file not found at {pt_model_path}")
        return
    ...
    if verbose:
        print("\n" + "="*80)
        print(f"HYBRID YOLO PREDICTION (PyTorch + ONNX) - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("="*80)
        ...
        onnx_model_size = os.path.getsize(onnx_model_path) / (1024 * 1024)
        print(f"\n📁 FILES:")
        print(f"  • PyTorch Model: {os.path.basename(pt_model_path)} ({pt_model_size:.2f} MB)")
        print(f"  • ONNX Model: {os.path.basename(onnx_model_path)} ({onnx_model_size:.2f} MB)")
    ...
    # Step 1: Run PyTorch model to get person detections
    if verbose:
        print(f"\n🔄 Loading PyTorch model from {pt_model_path}...")
    
    pt_model = YOLO(pt_model_path)
    
    if verbose:
        print(f"  • Running PyTorch inference for person detection...")
    
    # Run PyTorch model
    pt_results = pt_model(image_path, conf=conf_threshold, verbose=False)
    
    # Extract person detections from PyTorch results
    person_detections = []
    ...
    if verbose:
        print(f"  • Found {len(person_detections)} person detections with PyTorch model")
    
    # Step 2: Run ONNX model for all other detections
    if verbose:
        print(f"\n🔄 Loading ONNX model from {onnx_model_path}...")
    
    onnx_start_time = time.time()
    session = ort.InferenceSession(onnx_model_path, providers=['CPUExecutionProvider'])
    onnx_load_time = time.time() - onnx_start_time
    
    if verbose:
        print(f"  • Model loaded in {onnx_load_time:.2f} seconds")
    ...
    onnx_detections = postprocess(outputs, img_shape, preprocess_info, conf_threshold, iou_threshold)
    postprocess_time = time.time() - postprocess_start
    
    if verbose:
        print(f"  • Postprocessing time: {postprocess_time*1000:.2f} ms")
    
    # Filter out person detections from ONNX results (we'll use PyTorch's person detections)
    non_person_detections = [det for det in onnx_detections if int(det[5]) != 0]
    
    # Combine detections
    all_detections = person_detections + non_person_detections
    ...
    for detection in all_detections:
        ...
        # Draw bounding box with different colors based on model source
        if class_id == 0:  # Person class (from PyTorch)
            color = (0, 0, 255)  # Red for PyTorch detections
        else:  # Other classes (from ONNX)
            # Higher confidence = more green, lower confidence = more blue
            green = min(255, int(confidence * 255))
            blue = min(255, int((1 - confidence) * 255))
            color = (blue, green, 0)  # BGR format
    ...
    # Save the result
    output_path = f"output_hybrid_{os.path.basename(image_path)}"
    ...
    if verbose:
        ...
        print(f"  • PyTorch inference: included in total time")
        print(f"  • ONNX inference: {inference_time*1000:.2f} ms")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Hybrid YOLO object detection using PyTorch and ONNX models")
    parser.add_argument("--pt-model", type=str, default="models/yolo11n.pt", help="Path to YOLO PyTorch model file")
    parser.add_argument("--onnx-model", type=str, default="models/yolo11n.onnx", help="Path to YOLO ONNX model file")
    ...
    hybrid_predict(args.pt_model, args.onnx_model, args.image, args.conf, args.iou, verbose=not args.quiet)
